        ...
        // Phone Data (4 tiers: S, A, B, C - non-USB-C phones must be B tier or below)
        let phones = [
        ...
        function migrateData() {
            ...
            // Convert D tier person phone priorities to C (phones only have 4 tiers)
            people.forEach(p => {
                if (p.phoneTier === 'D') {
                    p.phoneTier = 'C';
                }
        ...
        // Enforce phone tier constraint: non-USB-C phones must be B tier or below
        function enforcePhoneTierConstraint(phone) {
            // Phones only have 4 tiers: S, A, B, C (no D)
            if (phone.tier === 'D') {
                phone.tier = 'C';
            }
            if (!phoneHasUSBC(phone)) {
                // Non-USB-C phones can only be B or C tier
                if (phone.tier === 'S' || phone.tier === 'A') {
                    phone.tier = 'B';
                }
            }
        }
        ...
        function renderPeopleGrid() {
            ...
            tiers.forEach(tier => {
                ...
                if (laptop) {
                    const pendingType = hasPendingSwap ? 'out' : null;
                    const laptopCard = createLaptopCard(laptop, {
                        showRemove: !hasPendingSwap,
                        showSwap: !hasPendingSwap,
                        pendingType,
            ...
            // Add spacer at end to allow scrolling past inventory panel
        ...
        function savePerson(e) {
            ...
            if (editingPersonIndex !== null) {
                people[editingPersonIndex] = { name, tier, phoneTier };
            } else {
                people.push({ name, tier, phoneTier });
            }
        ...
        function createPhoneCard(phone, options = {}) {
            ...
            let badgeHtml = '';
            if (pendingType === 'out' || pendingType === 'in') {
            ...
            div.innerHTML = `
                <div class="laptop-header">
                    <span class="laptop-model">${phone.model} ${badgeHtml}</span>
                    ${actionsHtml}
                </div>
                <div class="laptop-specs">
        ...
        function renderPhonePeopleGrid() {
            ...
            tiers.forEach(tier => {
                ...
                tierPeople.forEach(person => {
                    ...
                    card.innerHTML = `<div class="person-header tier-${phoneTier}"><span class="person-name">${person.name}</span><label class="lock-checkbox-container" title="Lock this phone assignment"><input type="checkbox" class="lock-checkbox" ${isLocked ? 'checked' : ''} /><span class="lock-icon">${isLocked ? '🔒' : '🔓'}</span></label><span class="tier-badge tier-${phoneTier}" data-person-name="${person.name}" data-phone-tier="true" style="cursor:pointer;" title="Click to change phone tier">${phoneTier}</span></div>`;
                    ...
                    if (phone) {
                        ...
                        if (hasPendingSwap) {
                            ...
                            if (newPhone) {
                                const incomingCard = createPhoneCard(newPhone, { pendingType: 'in' });
                        ...
                        slot.innerHTML = '<div class="empty-slot">Drop phone here</div>';
                    }

                    card.appendChild(slot);

                    // Add secured banner if locked
                    if (isLocked) {
                        const banner = document.createElement('div');
                    ...
                    card.addEventListener('drop', (e) => {
                        ...
                        if (draggedPhone && !pendingPhoneSwaps[person.name]) {
                            saveStateForUndo();
                            if (phoneDragSource !== 'inventory') {
                                logPhoneOwnershipChange(draggedPhone, null);
                                phoneAssignments[phoneDragSource] = null;
                            }
                            logPhoneOwnershipChange(draggedPhone, person.name);
                            phoneAssignments[person.name] = draggedPhone;
                            renderAll();
                        }
                    });
        ...
        function renderPhoneInventory() {
            ...
            const available = sortPhonesByTier(getAvailablePhones().filter(p => p.status !== 'incoming'));
            ...
            if (soonAvailable.length > 0) {
                ...
                const toSomeone = sortPhonesByTier(soonAvailable.filter(p => scheduledFor[p.id]));
                ...
                if (toSomeone.length > 0) {
                    ...
                    toSomeone.forEach(phone => {
                        const card = createPhoneCard(phone, { pendingType: 'soon', ownerName: null });
                        const header = card.querySelector('.phone-header');
                        const modelSpan = header.querySelector('.phone-model');
                        modelSpan.innerHTML += ` <span class="pending-badge scheduled">→ ${scheduledFor[phone.id]}</span>`;
                        goingGrid.appendChild(card);
                    });
        ...
        function savePhone(e) {
            ...
            if (editingPhoneId) {
                ...
                // Phones only have 4 tiers (S, A, B, C)
                if (tier === 'D') tier = 'C';
